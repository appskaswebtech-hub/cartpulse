...
export default function Outreach() {
    ...
    const FILTERS: { key: FilterKey; label: string }[] = [
        { key: "all", label: t.outreach.filterAll },
        { key: "not-contacted", label: t.outreach.notContacted },
        { key: "emailed", label: t.outreach.emailSentBadge },
        { key: "recovered", label: t.outreach.recovered },
        { key: "unsubscribed", label: t.outreach.unsubscribed },
    ];
        ...
        <Page>
            ...
            <div>
                ...
                <div style={{ marginBottom: "24px", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div>
                        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "4px" }}>
                            ...
                            <h1 style={{ fontSize: "22px", fontWeight: "700", margin: 0, color: "#111" }}>{t.outreach.title}</h1>
                        </div>
                        <p style={{ fontSize: "13px", color: "#9ca3af", margin: 0 }}>{t.outreach.subtitle}</p>
                    </div>
                    <div style={{ background: "#E1F5EE", borderRadius: "10px", padding: "8px 16px", fontSize: "13px", color: "#085041", fontWeight: "500" }}>
                        {carts.length} {t.outreach.customers}
                    </div>
                </div>
                    ...
                    <div style={{
                        ...
                        {result.success ? (
                            <>
                                ...
                                {t.outreach.emailsSentSuccess(result.sent)}
                                {result.skipped > 0 && ` ${t.outreach.skippedUnsub(result.skipped)}`}
                            </>
                        ) : (
                            <>
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" /></svg>
                                {t.outreach.emailError}
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "12px", marginBottom: "20px" }}>
                    {[
                        { bg: "#EEEDFE", color: "#534AB7", label: t.outreach.statTotal, value: stats.total, icon: <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" /> },
                        { bg: "#FEF9C3", color: "#854D0E", label: t.outreach.notContacted, value: stats.notContacted, icon: <><circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" /></> },
                        { bg: "#EEEDFE", color: "#534AB7", label: t.outreach.statEmailsSent, value: stats.totalEmailed, icon: <><line x1="22" y1="2" x2="11" y2="13" /><polygon points="22 2 15 22 11 13 2 9 22 2" /></> },
                        { bg: "#DCFCE7", color: "#166534", label: t.outreach.recovered, value: stats.recovered, icon: <><polyline points="23 4 23 10 17 10" /><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10" /></> },
                        { bg: "#FEE2E2", color: "#991b1b", label: t.outreach.unsubscribed, value: stats.totalUnsubscribed, icon: <><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></> },
                    ].map((s, i) => (
                    ...
                    {selectedIds.size > 0 && (
                        <div className="bulk-bar" style={{ marginLeft: "auto", borderRadius: "20px", padding: "6px 8px 6px 16px" }}>
                            <span style={{ fontSize: "13px", fontWeight: "600", color: "#fff", marginRight: "10px" }}>
                                {selectedIds.size} {t.outreach.selected}
                            </span>
                            <button
                                className="btn-send"
                ...
                <div className="cp-card" style={{ padding: 0, overflow: "hidden" }}>
                    <div style={{ padding: "18px 20px 14px", borderBottom: "1px solid #f0f0f0", display: "flex", alignItems: "center", gap: "10px" }}>
                        ...
                        <span style={{ fontSize: "15px", fontWeight: "600", color: "#111" }}>{t.outreach.customerOutreach}</span>
                        <span style={{ marginLeft: "auto", fontSize: "12px", color: "#9ca3af" }}>{filteredCarts.length} {t.outreach.customers}</span>
                    </div>
                    ...
                    {filteredCarts.length === 0 ? (
                        <div style={{ padding: "60px 24px", textAlign: "center" }}>
                            ...
                            <p style={{ fontSize: "15px", fontWeight: "600", color: "#374151", margin: "0 0 6px" }}>{t.outreach.noCustomersTitle}</p>
                            <p style={{ fontSize: "13px", color: "#9ca3af", margin: 0 }}>{t.outreach.noCustomersSubtitle}</p>
                        </div>
                    ) : (
                            ...
                            <table className="cp-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th>{t.outreach.customer}</th>
                                        <th>{t.outreach.products}</th>
                                        <th>{t.outreach.total}</th>
                                        <th>{t.outreach.abandonedOn}</th>
                                        <th>{t.outreach.emailHistory}</th>
                                        <th>{t.outreach.status}</th>
                                        <th>{t.outreach.action}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                            ...
                                            <tr key={cart.id} style={{ background: isSelected ? "#FAFAFF" : undefined }}>
                                                ...
                                                <td>
                                                    {emailNotifs.length === 0 ? (
                                                        <span className="badge-never">{t.outreach.never}</span>
                                                    ) : (
                                                ...
                                                <td>
                                                    {cart.isUnsubscribed ? (
                                                        <span className="badge-unsub">{t.outreach.unsubscribed}</span>
                                                    ) : cart.isRecovered ? (
                                                        <span className="badge-recovered">
                                                            <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12" /></svg>
                                                            {t.outreach.recovered}
                                                        </span>
                                                    ) : emailNotifs.length > 0 ? (
                                                        <span className="badge-emailed">{t.outreach.emailSentBadge}</span>
                                                    ) : (
                                                        <span className="badge-never">{t.outreach.notContacted}</span>
                                                    )}
                                                </td>
                                                <td>
                                                    {canSend && (
                                                        <button
                                                            ...
                                                            disabled={isSending}
                                                        >
                                                            {emailNotifs.length > 0 ? t.outreach.resend : (
                                                                <>
                ...
                <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) setUpgradeOpen(false); }}>
                    <div className="modal" style={{ maxWidth: "400px", padding: 0, overflow: "hidden" }}>
                        <div style={{ background: "linear-gradient(135deg,#534AB7,#7F77DD)", padding: "28px 28px 20px" }}>
                            ...
                            <h2 style={{ margin: "0 0 6px", fontSize: "18px", fontWeight: "700", color: "#fff" }}>{t.outreach.planLimitTitle}</h2>
                            <p style={{ margin: 0, fontSize: "13px", color: "rgba(255,255,255,0.75)" }}>
                                {t.outreach.planLimitDesc(planKey, planLimit)}
                            </p>
                        </div>
                ...
                <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) setModalOpen(false); }}>
                    <div className="modal">
                        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "20px" }}>
                            ...
                            <div>
                                <h2 style={{ margin: 0, fontSize: "16px", fontWeight: "700", color: "#111" }}>
                                    {modalCartIds.length > 1 ? t.outreach.sendToCustomers(modalCartIds.length) : t.outreach.modalTitle}
                                </h2>
                                <p style={{ margin: 0, fontSize: "12px", color: "#9ca3af" }}>{t.outreach.modalSubtitle}</p>
                            </div>
                            <button
                        ...
                        <div style={{ marginBottom: "14px" }}>
                            <label style={{ fontSize: "12px", fontWeight: "600", color: "#374151", display: "block", marginBottom: "6px" }}>{t.outreach.subjectLabel}</label>
                            <input className="modal-input" value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="Email subject line" />
                        </div>

                        <div style={{ marginBottom: "12px" }}>
                            <label style={{ fontSize: "12px", fontWeight: "600", color: "#374151", display: "block", marginBottom: "6px" }}>{t.outreach.messageLabel}</label>
                        ...
                        <div style={{ background: "#F8F7FF", borderRadius: "8px", padding: "10px 14px", marginBottom: "20px" }}>
                            <p style={{ margin: "0 0 4px", fontSize: "11px", color: "#534AB7", fontWeight: "600" }}>{t.outreach.variablesHint}</p>
                        ...
                        <div style={{ display: "flex", gap: "10px", justifyContent: "flex-end" }}>
                            ...
                            >
                                ...
                                {modalCartIds.length > 1 ? t.outreach.sendToCustomers(modalCartIds.length) : t.outreach.sendEmail}
                            </button>
                        </div>
                    </div>
                </div>
