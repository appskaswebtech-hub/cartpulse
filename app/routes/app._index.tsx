    ...
    <Page>
      ...
      <div style={{ padding: "0" }}>
        ...
        <div style={{ background: "linear-gradient(135deg, #1D9E75 0%, #085041 100%)", borderRadius: "16px", padding: "28px 32px", marginBottom: "24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <h1 style={{ fontSize: "26px", fontWeight: "700", margin: "0 0 4px", color: "#fff" }}>{t.dashboard.title}</h1>
            <p style={{ fontSize: "14px", color: "rgba(255,255,255,0.7)", margin: 0 }}>{t.dashboard.subtitle}</p>
          </div>
          <div style={{ display: "flex", gap: "24px" }}>
            <div style={{ textAlign: "center" }}>
              <p style={{ fontSize: "28px", fontWeight: "700", color: "#fff", margin: 0 }}>{stats.total}</p>
              <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.7)", margin: 0 }}>{t.dashboard.abandoned}</p>
            </div>
            <div style={{ width: "1px", background: "rgba(255,255,255,0.2)" }} />
            <div style={{ textAlign: "center" }}>
              <p style={{ fontSize: "28px", fontWeight: "700", color: "#fff", margin: 0 }}>{stats.totalNotifications}</p>
              <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.7)", margin: 0 }}>{t.dashboard.notified}</p>
            </div>
            <div style={{ width: "1px", background: "rgba(255,255,255,0.2)" }} />
            <div style={{ textAlign: "center" }}>
              <p style={{ fontSize: "28px", fontWeight: "700", color: "#fff", margin: 0 }}>{stats.recoveryRate}%</p>
              <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.7)", margin: 0 }}>{t.dashboard.recovery}</p>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", marginBottom: "16px" }}>
          ...
          <div className="cp-card">
            <div className="cp-stat" style={{ marginBottom: "20px" }}>
              ...
              <div>
                <p style={{ fontSize: "13px", color: "#9ca3af", margin: 0 }}>{t.dashboard.abandonedCartsChart}</p>
          ...
          <div className="cp-card">
            <div className="cp-stat" style={{ marginBottom: "20px" }}>
              ...
              <div>
                <p style={{ fontSize: "13px", color: "#9ca3af", margin: 0 }}>{t.dashboard.notificationsSentChart}</p>
        ...
        <div className="cp-card" style={{ marginBottom: "16px", display: "grid", gridTemplateColumns: "1fr auto", gap: "32px", alignItems: "center" }}>
          <div>
            <div className="cp-stat" style={{ marginBottom: "16px" }}>
              ...
              <div>
                <p style={{ fontSize: "13px", color: "#9ca3af", margin: 0 }}>{t.dashboard.recoveryRate}</p>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <p style={{ fontSize: "12px", color: "#6b7280", margin: 0 }}>{t.dashboard.recoveredCount(stats.recovered)}</p>
              <p style={{ fontSize: "12px", color: "#6b7280", margin: 0 }}>{t.dashboard.stillAbandonedCount(stats.total)}</p>
            </div>
          </div>
          <div style={{ background: "#EEEDFE", borderRadius: "12px", padding: "20px", minWidth: "220px" }}>
            ...
            <p style={{ fontSize: "12px", color: "#3C3489", margin: "0 0 12px", lineHeight: "1.6" }}>
              {stats.recovered === 0 ? t.dashboard.insightEmpty : t.dashboard.insightSome(stats.recovered)}
            </p>
          </div>
        </div>
        ...
        <div className="cp-card" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "16px" }}>
          {[
            { icon: "M3 4h18v2H3zM3 10h18v2H3zM3 16h18v2H3z", bg: "#EEEDFE", color: "#534AB7", label: t.dashboard.peakDay, value: peakDay.count > 0 ? peakDay.date : "—", sub: t.dashboard.mostActivity },
            { icon: "M12 2a10 10 0 1 0 0 20A10 10 0 0 0 12 2zM12 6v6l4 2", bg: "#E1F5EE", color: "#1D9E75", label: t.dashboard.notifications, value: stats.totalNotifications, sub: t.dashboard.totalSent },
            { icon: "M22 12h-4l-3 9L9 3l-3 9H2", bg: "#FAECE7", color: "#993C1D", label: t.dashboard.recoveryRate, value: `${stats.recoveryRate}%`, sub: t.dashboard.ofTotalCarts },
            { icon: "M23 6L13.5 15.5 8.5 10.5 1 18", bg: "#E1F5EE", color: "#1D9E75", label: t.dashboard.status, value: stats.total > 0 ? t.dashboard.active : t.dashboard.waiting, sub: stats.total > 0 ? t.dashboard.firingNotifications : t.dashboard.noCartsYet },
          ].map((item, i) => (
